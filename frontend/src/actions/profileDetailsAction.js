...
export const getProfileDetails = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `${LOCAL_HOST}/api/user/profile/update/${userInfo.username}`,
      config,
    );
...
export const profileUpdateDetailsAction =
  (email, username, first_name, last_name, phone_number, address) =>
  async (dispatch, getState) => {
    try {
      ...
      if (userInfo.email !== email) {
        moreData.email = email;
      }
      if (userInfo.username !== username) {
        moreData.username = username;
      }
...
export const userUpdatePasswordAction =
  (oldPassword, password, password2) => async (dispatch, getState) => {
    try {
      dispatch({
      ...
      const { data } = await axios.put(
        `${LOCAL_HOST}/api/user/update_password/`,
        {
